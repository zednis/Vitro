...
#make lines with additional query_field info
s|\(.*\) \(.*\) .* \[\(.*\)\] "\(.*\)" \(.*\) \(.*\) "\(.*\)" "\(.*\)" \(.*\)|insert ignore into log (client_ip, client_host, reply_date, request, server_status, reply_size, ref, agent, query_field, full_log_line, page_type) values ( '\1', '\2', '\3', '\4', \5, \6, '\7', '\8', '\9', '&', 'fedsearch') ;|
